...
class GameOver(arcade.View):
    def __init__(self):
        super().__init__()
        self.texture =arcade.load_texture(":C:User/Pasha/Desktop/DAL/stoneMid.png")
...
class fragMons(arcade.Sprite):
    def follow_sprite (self,player_sprite):
        if self.center_y < player_sprite.center_y:
            self.center_y += min(4, player_sprite.center_y - self.center_y)
        elif self.center_y > player_sprite.center_y:
            self.center_y -= min(4, self.center_y - player_sprite.center_y)

        if self.center_x < player_sprite.center_x:
            self.center_x += min(4, player_sprite.center_x - self.center_x)
        elif self.center_x > player_sprite.center_x:
            self.center_x -= min(4, self.center_x - player_sprite.center_x)
class GameView(arcade.View):
    ...
    def setup(self):
        ...
        planet.center_y= random.randrange(130,SCREEN_HEIGHT)
        self.planet_list.append(planet)
        for i in range(5):
            texture="images/monster.png"
            self.enemy = meeleMons(texture, 0.1,self.mons_bar_list)
    ...
    def on_update(self, delta_time):
        ...
        for planet in self.planet_list:
            ...
            if self.score>0:
                if self.mons_timer >=3:
                    self.mons_timer =0
                    if len (hit_list) >0:
                        self.player_sprite.health += 2.5
                        self.score -=1
                        self.player_sprite.indicator_bar.fullness = (
                        self.player_sprite.health / PLAYER_HEALTH
                        )
        ...
        for enemy in self.mons_list:
            ...
            if len(self.mons_list) ==0:
                    texture="images/monster.png"
                    new_enemy = meeleMons(texture, 0.1,self.mons_bar_list)
                    new_enemy.center_x = random.randrange(SCREEN_WIDTH)
                    new_enemy.center_y = random.randrange(SCREEN_HEIGHT)
            if new_enemy:
                self.mons_list.append(new_enemy[0])
                self.mons_list.append(new_enemy[1])
